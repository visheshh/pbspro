...
int
decode_DIS_SchedCycleEnd(int sock, struct batch_request *preq)
{
	int rc;

	preq->rq_ind.rq_sched_cycle_end.rq_scname = disrst(sock, &rc);

	if (rc)
		return rc;

	preq->rq_ind.rq_sched_cycle_end.rq_start_or_stop = disrsi(sock, &rc);
	return rc;
