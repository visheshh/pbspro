...
set_resc_assigned(void *pobj, int objtype, enum batch_op op)
{
	...
	if (objtype == 1)
		update_job_node_rassn(presv, objtype, &presv->ri_wattr[(int)RESV_ATR_resv_nodes], op);
		...
		if (pjob->ji_wattr[(int) JOB_ATR_resc_released].at_flags & ATR_VFLAG_SET)
			...
			update_job_node_rassn(pjob, objtype, &pjob->ji_wattr[(int) JOB_ATR_resc_released], op);
		else
			/* updating all resources from exec vnode attribute */
			update_job_node_rassn(pjob, objtype, &pjob->ji_wattr[(int) JOB_ATR_exec_vnode], op);
		if (pjob->ji_wattr[(int)JOB_ATR_exec_vnode_deallocated].at_flags & ATR_VFLAG_SET) {
			update_job_node_rassn(pjob, objtype, &pjob->ji_wattr[(int) JOB_ATR_exec_vnode_deallocated], op);
		}
	}
}
...
ck_chkpnt(attribute *pattr, void *pobject, int mode)
{
	...
	if (mode == ATR_ACTION_ALTER) {
		pque = find_queuebyname(((job *)pobject)->ji_qs.ji_queue, 0);
