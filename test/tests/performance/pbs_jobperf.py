...
class TestJobPerf(TestPerformance):
    ...
    @testparams(num_jobs_scof=100, num_users_scof=25,
                num_moms_scof=25, num_ncpus_scof=100)
    @timeout(3600)
    def test_job_performance_sched_off(self):
        """
        Test Job subission rate when scheduling is off by
        submitting 2.5k jobs and turn on scheduling with 2.5k ncpus.
        """
        ...
        job += ' -- /bin/true '
        start = time.time()
        result = self.run_paralell(num_users, job, num_jobs)
    ...
    def test_job_performance_sched_on(self):
        ...
        while i < self.num_iter:
            ...
            job += ' -- /bin/true '
            start = time.time()
            result = self.run_paralell(self.num_users, job, self.num_jobs)
            res = statistics.mean(result)
            resps = (self.num_jobs *
                     self.num_users) / res
    ...
    def test_qstat_perf(self):
        ...
        while i < num_iter:
            ...
            stat_cmd += ' -f'
            start = time.time()
            stat_time = self.run_paralell(num_users, stat_cmd, num_qstats)
            i = i + 1
    ...
    def test_qstat_hist_perf(self):
        ...
        while i < self.num_iter:
            ...
            stat_cmd += ' -f '
            start = time.time()
            stat_time = self.run_paralell(self.num_users, stat_cmd, num_qstats)
            i = i + 1
        a = {'job_history_enable': False}
        self.server.manager(MGR_CMD_SET, SERVER, a)
    ...
    @testparams(num_jobs_rk=100, num_users_rk=10,
                num_iter_rk=10)
    @timeout(3600)
    ...
    @testparams(num_jobs_r=100, num_users_r=10,
                num_iter_r=10)
    @timeout(3600)
    ...
    @testparams(num_jobs_qd=100, num_users_qd=10)
    @timeout(3600)
    ...
    @testparams(num_jobs_qdh=100, num_users_qdh=25,
